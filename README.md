...
| **Home** | `HomeScreen` | A **"For you"** card at the top (the single most time-sensitive thing across all groups — overdue/upcoming attendance, a cascade needing a Captain, or an open vote); groups list with a **live status** per row ("2 unread · vote open"); header (bell badging only actionable alerts, avatar menu), greeting, real **search**, and the **AI assistant** button. |
| **Alerts** | `AlertsScreen` | Two buckets — **"Needs you"** (deadline/action items: open votes, missed check-ins, reports — the only ones that badge the bell) and **"Other"** (informational). Tap marks read and jumps to the vote/feed/attendance/reports. |
...
the group's **Media & links** tab. **Offline** (first pass, via
`@react-native-community/netinfo`): an offline banner shows app-wide, and a Feed
message sent while offline is queued locally (shown as "queued · [time]") and
sent automatically on reconnect; votes/attendance just report you're offline. When a member returns to a busy chat with a
